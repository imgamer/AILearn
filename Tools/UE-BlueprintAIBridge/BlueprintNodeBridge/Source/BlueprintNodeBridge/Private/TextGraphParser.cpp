...
FString FTextGraphParser::GenerateShortCode(const TArray<UEdGraphNode*>& Nodes, TMap<FString, FString>& OutNodeCache)
{
	...
	// Export all nodes together to preserve connections
	...
	for (int32 i = 0; i < Nodes.Num(); i++)
	{
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			...
			{
				FString DisplayName = GetExecPinName(PinName, true);
				FString Value = GetPinValue(Pin);
				if (!Value.IsEmpty())
				{
					Inputs += FString::Printf(TEXT("%s\\(%s\\); "), *DisplayName, *EscapePinValue(Value));
				}
				else
				{
			...
			{
				FString DisplayName = GetExecPinName(PinName, false);
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			...
			for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
			{
				...
				if (NodeToID.Contains(TargetNode))
				{
					...
					FString SourcePinName = GetExecPinName(Pin->PinName.ToString(), false);
					FString TargetPinName = GetExecPinName(LinkedPin->PinName.ToString(), true);
...
bool FTextGraphParser::ParseAndPaste(const FString& ShortCode, UBlueprint* Blueprint, const TMap<FString, FString>& NodeCache, FVector2D Location, UEdGraph* InTargetGraph)
{
	...
	FScopedTransaction Transaction(NSLOCTEXT("BlueprintNodeBridge", "PasteNodes", "Paste Nodes"));
	InTargetGraph->Modify();

	TMap<FString, UEdGraphNode*> CreatedNodes;

	// Regenerate GUIDs to force new node creation
	...
	for (const FLinkDefinition& Link : Links)
	{
		...
		FName SourcePinFName = FName(*Link.SourcePinName);
		FName TargetPinFName = FName(*Link.TargetPinName);
...
bool FTextGraphParser::IsValidShortCode(const FString& ShortCode)
{
...
FString FTextGraphParser::GetExecPinName(const FString& PinName, bool bIsInput)
{
	if (PinName == UEdGraphSchema_K2::PN_Execute.ToString() && bIsInput)
		return TEXT("inExec");
	if (PinName == UEdGraphSchema_K2::PN_Then.ToString() && !bIsInput)
		return TEXT("outExec");
	return PinName;
}
